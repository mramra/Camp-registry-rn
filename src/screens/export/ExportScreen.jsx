    ...
    try {
      ...
      const dataRows = sorted.map((f) => {
        ...
        return [
          ...
          members.length + 1,
          f.head_marital || '',
          f.income_source || 'لا يعمل', // قيمة افتراضية فقط لو الحقل فاضي (أسرة لسه ما اتعبّت)
          ages.c06,
          ages.c612,
          ages.c1218,
          ages.adult,
          ages.elderly,
          f.address_details || f.original_address || '',
          f.original_address || 'غزة',
          f.address || campInfo.name || '',
          f.governorate_current || 'غزة',
          f.displacement_status || 'نازح',
          f.housing_type || 'خيمة',
          f.housing_condition || 'هدم كلي',
          chronicCount,
          disabilityCount,
          injuryCount,
          f.notes || '',
            ...
            {canExport ? (
              <>
                <Text style={styles.compNote}>
                  يبني كشفاً بنفس تخطيط القالب المرفوع حرفياً لمخيم واحد محدَّد. الأعمدة (دخل/
                  محافظة/نزوح/نوع وحالة السكن) صارت حقولاً فعلية بنموذج الأسرة — تُعرض قيمتها
                  الحقيقية لو عُبِّئت، وتُستخدم قيمة افتراضية (لا يعمل/غزة/نازح/خيمة/هدم كلي) فقط
                  للأسر القديمة اللي لسه ما اتعبّت هالحقول.
                </Text>
