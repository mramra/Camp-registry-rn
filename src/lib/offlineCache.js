...
/**
 * مهلة زمنية صريحة لأي عملية شبكة (فحص اتصال أو جلب بيانات) — بعض حالات
 * الشبكة (واي فاي متصل لكن بلا إنترنت فعلي، أو تغطية ضعيفة جداً بالمخيمات)
 * تخلي طلبات الشبكة "تعلّق" لفترة طويلة جداً بدل ما تفشل بسرعة، فتفضل
 * دائرة التحميل تلف بلا نهاية بدل ما ترجع فوراً للنسخة المحفوظة. هذي
 * الدالة تضمن إن أي عملية تفشل خلال مهلة محددة (8 ثواني افتراضياً) حتى
 * لو الشبكة نفسها ما ردّت بشي.
 */
export const withTimeout = (promise, ms = 8000, message = 'انتهت مهلة الاتصال') =>
  Promise.race([
    promise,
